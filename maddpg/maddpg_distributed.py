...
class MADDPG:
    def __init__(
        ...
        actor_obs_shape = obs_shape[agent_id]
        actor_action_shape = action_shape[agent_id]

        # critic is centralized and can see all observations and actions
        critic_obs_shape = obs_shape
        critic_action_shape = action_shape

        # create the network
        self.actor_network = Actor(
            args.high_action, actor_obs_shape, actor_action_shape
        )
        self.critic_network = Critic(
            args.high_action, critic_obs_shape, critic_action_shape
        )
    ...
    def train(self, transitions, u_next):
        ...
        # Save temporary model at every train
    ...
    def save_temp_model(self):
        ...
        if not os.path.exists(model_path):
            os.makedirs(model_path)
        torch.save(
            self.actor_network.state_dict(), model_path + "/temp_actor_params.pkl"
        )
        torch.save(
            self.critic_network.state_dict(), model_path + "/temp_critic_params.pkl"
        )
