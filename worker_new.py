...
def update_target_networks(**kwargs):
    ...
    for agent_id, agent in enumerate(runner.agents):
        ...
        for param, target_actor_param in zip(agent.policy.actor_target_network.parameters(), actor[agent_id]):
        ...
        for param, target_critic_param in zip(agent.policy.critic_target_network.parameters(), critic[agent_id]):
